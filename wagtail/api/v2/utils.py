...
def parse_fields_parameter(fields_str):
    ...
    fields, remaining = parse_fields(fields_str)

    if remaining:
        # The parser quit too early so there must be an extra close bracket
        raise BadRequestError("unexpected char ')' at position %d" % get_position(remaining))
