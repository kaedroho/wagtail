...
from wagtail.videos import get_video_model, get_video_model_string
from wagtail.videos.fields import WagtailVideoField
from wagtail.videos.formats import Format, get_video_format, register_video_format
from wagtail.videos.forms import get_video_form
from wagtail.videos.models import Video as WagtailVideo
from wagtail.videos.rect import Rect, Vector
from wagtail.videos.utils import generate_signature, verify_signature
from wagtail.videos.views.serve import ServeView
from wagtail.tests.testapp.models import CustomVideo, CustomVideoFilePath
...
class TestVideoTag(TestCase):
    ...
    def render_video_tag(self, video, filter_spec):
        temp = template.Template(
            "{% load wagtailvideos_tags %}{% video video_obj " + filter_spec + "%}"
        )
        context = template.Context({"video_obj": video})
        return temp.render(context)

    def test_video_tag(self):
        result = self.render_video_tag(self.video, "width-400")

        # Check that all the required HTML attributes are set
        self.assertIn('width="400"', result)
        self.assertIn('height="300"', result)
        self.assertIn('alt="Test video"', result)

    def test_video_tag_none(self):
        result = self.render_video_tag(None, "width-500")
        self.assertEqual(result, "")

    def test_video_tag_wrong_type(self):
        with self.assertRaises(ValueError):
            self.render_video_tag("foobar", "width-500")

    def render_video_tag_as(self, video, filter_spec):
        temp = template.Template(
    ...
    def test_video_tag_attrs(self):
        result = self.render_video_tag_as(self.video, "width-400")
    ...
    def render_video_tag_with_extra_attributes(self, video, title):
        temp = template.Template(
            '{% load wagtailvideos_tags %}{% video video_obj width-400 \
            class="photo" title=title|lower alt="Alternate" %}'
        )
        context = template.Context({"video_obj": video, "title": title})
        return temp.render(context)

    def test_video_tag_with_extra_attributes(self):
        result = self.render_video_tag_with_extra_attributes(
            self.video, "My Wonderful Title"
        )
    ...
    def render_video_tag_with_filters(self, video):
        temp = template.Template(
            "{% load wagtailvideos_tags %}{% video video_primary|default:video_alternate width-400 %}"
        )
        context = template.Context({"video_primary": None, "video_alternate": video})
        return temp.render(context)

    def test_video_tag_with_filters(self):
        result = self.render_video_tag_with_filters(self.video)
        self.assertIn('width="400"', result)
        self.assertIn('height="300"', result)

    def test_video_tag_with_chained_filters(self):
        result = self.render_video_tag(self.video, "fill-200x200 height-150")
    ...
    def test_filter_specs_must_match_allowed_pattern(self):
        with self.assertRaises(template.TemplateSyntaxError):
            self.render_video_tag(self.video, "fill-200x200|height-150")

        with self.assertRaises(template.TemplateSyntaxError):
            self.render_video_tag(self.video, 'fill-800x600 alt"test"')

    def test_context_may_only_contain_one_argument(self):
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                "{% load wagtailvideos_tags %}{% video video_obj fill-200x200"
                " as test_img this_one_should_not_be_there %}<img {{ test_img.attrs }} />"
            )
            context = template.Context({"video_obj": self.video})
            temp.render(context)

    def test_no_video_filter_provided(self):
        # if video template gets the video but no filters
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                "{% load wagtailvideos_tags %}{% video video_obj %}"
            )
    ...
    def test_no_video_filter_provided_when_using_as(self):
        # if video template gets the video but no filters
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                "{% load wagtailvideos_tags %}{% video video_obj as foo %}"
            )
            context = template.Context({"video_obj": self.video})
            temp.render(context)

    def test_no_video_filter_provided_but_attributes_provided(self):
        # if video template gets the video but no filters
        with self.assertRaises(template.TemplateSyntaxError):
            temp = template.Template(
                '{% load wagtailvideos_tags %}{% video video_obj class="cover-video"%}'
            )
            context = template.Context({"video_obj": self.video})
            temp.render(context)

    def render_video_url_tag(self, video, view_name):
        temp = template.Template(
            '{% load wagtailvideos_tags %}{% video_url video_obj "width-400" "'
            + view_name
            + '" %}'
        )
        context = template.Context({"video_obj": video})
    ...
    def test_video_url(self):
        result = self.render_video_url_tag(self.video, "wagtailvideos_serve")
        self.assertRegex(
            result,
            "/videos/.*/width-400/{}".format(self.video.file.name.split("/")[-1]),
        )

    def test_video_url_custom_view(self):
        result = self.render_video_url_tag(
            self.video, "wagtailvideos_serve_custom_view"
        )

        self.assertRegex(
            result,
            "/testvideos/custom_view/.*/width-400/{}".format(
                self.video.file.name.split("/")[-1]
            ),
        )

    def test_video_url_no_videoserve_view_added(self):
        # if video_url tag is used, but the video serve view was not defined.
        with self.assertRaises(ImproperlyConfigured):
            temp = template.Template(
                '{% load wagtailvideos_tags %}{% video_url video_obj "width-400" "mynonexistingvideoserve_view" %}'
            )
            context = template.Context({"video_obj": self.video})
            temp.render(context)


class TestMissingVideo(TestCase):
    """
    Missing video files in media/original_videos should be handled gracefully, to cope with
    pulling live databases to a development instance without copying the corresponding video files.
    In this case, it's acceptable to render broken videos, but not to fail rendering the page outright.
    """

    fixtures = ["test.json"]

    def test_video_tag_with_missing_video(self):
        # the page /events/christmas/ has a missing video as the feed video
        response = self.client.get("/events/christmas/")
        self.assertContains(
            response,
            '<img src="/media/not-found" width="0" height="0" alt="A missing video" class="feed-video">',
            html=True,
        )

    def test_rich_text_with_missing_video(self):
        # the page /events/final-event/ has a missing video in the rich text body
        response = self.client.get("/events/final-event/")
        self.assertContains(
            response,
            '<img class="richtext-video full-width" src="/media/not-found" \
            width="0" height="0" alt="where did my video go?">',
            html=True,
        )
...
class TestFormat(TestCase, WagtailTestUtils):
    ...
    def test_editor_attributes(self):
        result = self.format.editor_attributes(self.video, "test alt text")
        self.assertEqual(
            result,
            {
                ...
                "data-id": self.video.pk,
            },
        )

    def test_video_to_editor_html(self):
        result = self.format.video_to_editor_html(self.video, "test alt text")
        self.assertTagInHTML(
            '<img data-embedtype="video" data-id="%d" data-format="test name" '
            'data-alt="test alt text" class="test classnames" '
            'width="640" height="480" alt="test alt text" >' % self.video.pk,
            result,
            allow_extra_attrs=True,
        )

    def test_video_to_editor_html_with_quoting(self):
        result = self.format.video_to_editor_html(
            self.video, 'Arthur "two sheds" Jackson'
        )
        expected_html = (
            '<img data-embedtype="video" data-id="%d" data-format="test name" '
    ...
    def test_video_to_html_no_classnames(self):
        self.format.classnames = None
        result = self.format.video_to_html(self.video, "test alt text")
        self.assertTagInHTML(
    ...
    def test_video_to_html_with_quoting(self):
        result = self.format.video_to_html(self.video, 'Arthur "two sheds" Jackson')
        self.assertTagInHTML(
    ...
    def test_get_video_format(self):
        register_video_format(self.format)
        result = get_video_format("test name")
...
class TestSignatureGeneration(TestCase):
    ...
    def test_signature_changes_on_video_id(self):
        self.assertFalse(
...
class TestFrontendServeView(TestCase):
    ...
    def test_get(self):
        ...
        """
        # Generate signature
        signature = generate_signature(self.video.id, "fill-800x600")

        # Get the video
        response = self.client.get(
            reverse(
                "wagtailvideos_serve", args=(signature, self.video.id, "fill-800x600")
            )
        )
    ...
    def test_get_with_serve_action(self):
        ...
        response = self.client.get(
            reverse(
                "wagtailvideos_serve_action_serve",
                args=(signature, self.video.id, "fill-800x600"),
            )
        )
    ...
    def test_get_with_redirect_action(self):
        ...
        response = self.client.get(
            reverse(
                "wagtailvideos_serve_action_redirect",
                args=(signature, self.video.id, "fill-800x600"),
            )
        )

        expected_redirect_url = (
            "/media/videos/{filename[0]}.2e16d0ba.fill-800x600{filename[1]}".format(
                filename=os.path.splitext(os.path.basename(self.video.file.path))
            )
        )
    ...
    def test_get_with_custom_key(self):
        ...
        """
        # Generate signature
        signature = generate_signature(self.video.id, "fill-800x600", key="custom")

        # Get the video
        response = self.client.get(
            reverse(
                "wagtailvideos_serve_custom_key",
                args=(signature, self.video.id, "fill-800x600"),
            )
            + "test.png"
        )
    ...
    def test_get_invalid_signature(self):
        ...
        """
        # Generate a signature for the incorrect video id
        signature = generate_signature(self.video.id + 1, "fill-800x600")
    ...
    def test_get_invalid_filter_spec(self):
        ...
        signature = generate_signature(self.video.id, "bad-filter-spec")

        # Get the video
        response = self.client.get(
            reverse(
                "wagtailvideos_serve",
                args=(signature, self.video.id, "bad-filter-spec"),
            )
        )
    ...
    def test_get_missing_source_video_file(self):
        """
        Test that a missing video file gives a 410 response

        When the source video file is missing, it is presumed deleted so we
...
class TestGetVideoForm(TestCase, WagtailTestUtils):
    ...
    def test_admin_form_fields_attribute(self):
        form = get_video_form(CustomVideo)
    ...
    def test_file_field(self):
        form = get_video_form(WagtailVideo)

        self.assertIsInstance(form.base_fields["file"], WagtailVideoField)
...
class TestRenditionFilenames(TestCase):
    # Can't create video in setUp as we need a unique filename for each test.
    ...
    def test_normal_filter(self):
        video = Video.objects.create(
            title="Test video",
            file=get_test_video_file(filename="test_rf1.png"),
        )
        rendition = video.get_rendition("width-100")

        self.assertEqual(rendition.file.name, "videos/test_rf1.width-100.png")

    def test_fill_filter(self):
        video = Video.objects.create(
            title="Test video",
            file=get_test_video_file(filename="test_rf2.png"),
        )
        rendition = video.get_rendition("fill-100x100")

        self.assertEqual(
            rendition.file.name, "videos/test_rf2.2e16d0ba.fill-100x100.png"
        )

    def test_fill_filter_with_focal_point(self):
        video = Video.objects.create(
            title="Test video",
            file=get_test_video_file(filename="test_rf3.png"),
        )
        video.set_focal_point(Rect(100, 100, 200, 200))
        video.save()

        rendition = video.get_rendition("fill-100x100")

        self.assertEqual(
            rendition.file.name, "videos/test_rf3.15ee4958.fill-100x100.png"
        )

    def test_filter_with_pipe_gets_dotted(self):
        video = Video.objects.create(
            title="Test video",
            file=get_test_video_file(filename="test_rf4.png"),
        )
        video.set_focal_point(Rect(100, 100, 200, 200))
        video.save()

        rendition = video.get_rendition("fill-200x200|height-150")

        self.assertEqual(
            rendition.file.name, "videos/test_rf4.15ee4958.fill-200x200.height-150.png"
        )


class TestDifferentUpload(TestCase):
    def test_upload_path(self):
        video = CustomVideoFilePath.objects.create(
            title="Test video",
            file=get_test_video_file(),
        )

        second_video = CustomVideoFilePath.objects.create(
            title="Test Video",
            file=get_test_video_file(colour="black"),
        )

        # The files should be uploaded based on it's content, not just
        # it's filename
        self.assertNotEqual(video.file.url, second_video.file.url)
...
class TestGetVideoModel(WagtailTestUtils, TestCase):
    ...
    def test_custom_get_video_model(self):
        """Test get_video_model with a custom video model"""
        self.assertIs(get_video_model(), CustomVideo)

    @override_settings(WAGTAILIMAGES_IMAGE_MODEL="tests.CustomVideo")
    def test_custom_get_video_model_string(self):
        """Test get_video_model_string with a custom video model"""
        self.assertEqual(get_video_model_string(), "tests.CustomVideo")

    @override_settings()
    def test_standard_get_video_model(self):
        """Test get_video_model with no WAGTAILIMAGES_IMAGE_MODEL"""
    ...
    def test_standard_get_video_model_string(self):
        """Test get_video_model_STRING with no WAGTAILIMAGES_IMAGE_MODEL"""
        del settings.WAGTAILIMAGES_IMAGE_MODEL
