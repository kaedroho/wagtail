...
from wagtail.videos.utils import parse_color_string
...
class VideoTransform:
    """
    Tracks transformations that are performed on an video.
    ...
    features such as transforming the focal point of the video.
    """
    ...
    def clone(self):
        clone = VideoTransform(self.size)
    ...
    def crop(self, rect):
        ...
        # Transform the video so the top left of the rect is at (0, 0), then set the size
        clone = self.clone()
        clone.offset = (
    ...
    def transform_vector(self, vector):
        """
        Transforms the given vector into the coordinate space of the final video.

        Use this to find out where a point on the source video would end up in the
        final video after cropping/resizing has been performed.
    ...
    def untransform_vector(self, vector):
        """
        Transforms the given vector back to the coordinate space of the source video.

        This performs the inverse of `transform_vector`. Use this to find where a point
        in the final cropped/resized video originated from in the source video.
    ...
    def get_rect(self):
        """
        Returns a Rect representing the region of the original video to be cropped.
        """
        return Rect(
            -self.offset[0],
            -self.offset[1],
...
class FillOperation(TransformOperation):
    ...
    def run(self, transform, video):
        video_width, video_height = transform.size
        focal_point = video.get_focal_point()

        # Get crop aspect ratio
        crop_aspect_ratio = self.width / self.height

        # Get crop max
        crop_max_scale = min(video_width, video_height * crop_aspect_ratio)
        ...
        if focal_point is not None:
            ...
            # Sometimes, the focal point may be bigger than the video...
        ...
        fp_u = fp_x / video_width
        fp_v = fp_y / video_height
        ...
        # Don't allow the crop box to go over the video boundary
        rect = rect.move_to_clamp(Rect(0, 0, video_width, video_height))
        ...
        # Only resize if the video is too big
        if scale < 1.0:
            # Resize!
...
class MinMaxOperation(TransformOperation):
    ...
    def run(self, transform, video):
        ...
        horz_scale = self.width / video_width
        vert_scale = self.height / video_height

        if self.method == "min":
            if video_width <= self.width or video_height <= self.height:
                return transform

            if horz_scale > vert_scale:
                width = self.width
                height = int(video_height * horz_scale)
            else:
                width = int(video_width * vert_scale)
                height = self.height
...
class WidthHeightOperation(TransformOperation):
    ...
    def run(self, transform, video):
        ...
        if self.method == "width":
            if video_width <= self.size:
                return transform

            scale = self.size / video_width

            width = self.size
            height = int(video_height * scale)

        elif self.method == "height":
            if video_height <= self.size:
                return transform

            scale = self.size / video_height
