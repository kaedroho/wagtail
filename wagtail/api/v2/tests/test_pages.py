...
class TestPageListing(TestCase):
    ...
    def test_locale_filter_with_i18n_disabled(self):
        response = self.get_response(locale='en')
        content = json.loads(response.content.decode('UTF-8'))

        self.assertEqual(response.status_code, 400)
        self.assertEqual(content, {'message': "internationalisation features are disabled"})
