...
Passing ``random`` into the ``?order`` parameter cause results to be ordered
randomly.
...
Any field may be used for an exact match filter. Use the filter name as the
...
By default, only a subset of the available fields are returned by the listing
view. The ``?fields`` parameter can be used to both add additional fields to the
...
For example, ``?fields=-title,body`` will remove the ``title`` and add the ``body``.

This can also be used in with the asterisk. For example, ``?fields=*,-body``
includes all fields except for the ``body``.

Removing all default fields
^^^^^^^^^^^^^^^^^^^^^^^^^^^

If you would like to specify exactly the fields you need, you set the first item
in fields to an underscore (``_``) which removes all default fields.

For example, ``?fields=_,title`` will only include the title field.
