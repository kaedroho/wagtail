...
class ChildRelationComparison:
    ...
    def get_mapping(self, objs_a, objs_b):
        """
        Builds a mapping between the old child objects and the new. This
        bit of code attempts to match the objects in the old revision with
        their counterpart in the new revision.
        ...
        This returns 4 values:
         - map_forwards => a mapping of object indexes from the old version to the new version
         - map_backwards => a mapping of object indexes from the new version to the old version
         - new => a list of indices for objects that didn't exist in the old version
         - deleted => a list of indices for objects that didn't exist in the new version
    ...
    def get_child_comparisons(self):
        ...
        They are returned in the order they appear in the new version with deletions
        appended at the end.
...
class ChildObjectComparison:
    def __init__(self, model, old_version, new_version, field_comparisons):
        self.model = model
    ...
    def is_addition(self):
        """
        Returns True if this child object was created since old_version
        """
        return self.new_version and not self.old_version

    def is_deletion(self):
        """
        Returns True if this child object was deleted in new_version
        """
        return self.old_version and not self.new_version
    ...
    def get_position_change(self):
        ...
        if not self.is_addition() and not self.is_deletion():
            return getattr(self.new_version, 'sort_order', 0) - getattr(self.old_version, 'sort_order', 0)
    ...
    def get_field_comparisons(self):
        ...
        if self.is_addition() or self.is_deletion():
            ...
            obj = self.old_version or self.new_version

            for field_comparison in self.field_comparisons:
                comparisons.extend(field_comparison(obj, obj))
        else:
            for field_comparison in self.field_comparisons:
                comparisons.extend(field_comparison(self.old_version, self.new_version))
