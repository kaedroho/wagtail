...
def get_fill_filter_spec_migrations(app_name, rendition_model_name):
    def fill_filter_spec_forward(apps, schema_editor):
        ...
        Filter = apps.get_model("wagtailvideos", "Filter")
...
def generate_signature(video_id, filter_spec, key=None):
    if key is None:
    ...
    url = "{}/{}/".format(video_id, filter_spec)
    return force_str(
        base64.urlsafe_b64encode(hmac.new(key, url.encode(), hashlib.sha1).digest())
    )


def verify_signature(signature, video_id, filter_spec, key=None):
    return force_str(signature) == generate_signature(video_id, filter_spec, key=key)
