...
from django.db.models import Avg, Count, F, FloatField, Manager, Q, TextField, Value
from django.db.models.constants import LOOKUP_SEP
from django.db.models.functions import Cast, Length, Log
...
class ObjectIndexer:
    ...
    def title(self):
        """
        Returns all values to index as "title". This is the value of all SearchFields that have title=True
        """
        texts = []
    ...
    def body(self):
        ...
        for field in self.search_fields:
            for current_field, boost, value in self.prepare_field(self.obj, field):
                if isinstance(current_field, SearchField) and not current_field.partial_match and not current_field.field_name == 'title':
                    texts.append((value, boost))
    ...
    def autocomplete(self):
        ...
        for field in self.search_fields:
            for current_field, boost, value in self.prepare_field(self.obj, field):
                # FIXME: I know it's wrong to exclude title here. We're not demoing htis and the autocomplete PR should set this right
